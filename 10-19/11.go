...
func Euler11() {
	...
	//Calculate down diagonal products
	for i := 0; i < 17; i++ {
		for j := 0; j < 17; j++ {
	...
	//Calculate up diagonal products
	for i := 3; i < 20; i++ {
		for j := 0; j < 17; j++ {
			product := input[i][j] * input[i-1][j+1] * input[i-2][j+2] * input[i-3][j+3]
...
func GetInputMatrix() [20][20]int {
	...
	numsArray := strings.Split(inputText, " ")

	inputMatrix := [20][20]int{}

	for i := 0; i < 20; i++ {
		for j := 0; j < 20; j++ {

			inputMatrix[i][j], _ = strconv.Atoi(numsArray[i*20+j])
		}
	}
